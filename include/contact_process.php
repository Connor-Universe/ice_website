...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($_POST["first_name"])) {
    ...
    if (!preg_match("/^[a-zA-Z ]*$/",$last_name)) {
      $first_name_err = "<span class='error'>Only letters & white space!</span>";
    }
  }
  if (empty($_POST["last_name"])) {
    ...
    // check if name only contains letters and whitespace
    if (!preg_match("/^[a-zA-Z ]*$/",$last_name)) {
      $last_name_err = "<span class='error'>Only letters & white space!</span>";
    }
  }
...
if (!$mail->send()) {
  ...
  $fail   = "<span class='contact-fail'>Email not sent! If problem persist reach us on whatsapp!</span>"; 
     ...
     $success= "<span class='contact-success'>Message has sent,thank you $first_name for contacting ice24updates.com!</span> ";
